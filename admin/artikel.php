...
<?php if (empty($artikel_liste)): ?>
  <p style="color:#666;font-size:.9rem">Noch keine Themenartikel vorhanden. Klicke oben auf „+ Neuer Artikel“, um den ersten Beitrag zu verfassen.</p>
<?php else: ?>
  ...
  <?php foreach ($artikel_liste as $art): ?>
    <tr class="<?= !$art['is_published'] ? 'inactive' : '' ?>">
      <td data-label="Titel">
        ...
        <div style="font-size:.8rem;color:#777;margin-top:.2rem;">
          <code>/themen/<?= e($art['slug']) ?></code>
        </div>
      </td>
      ...
      <td data-label="Aktionen">
        <div class="actions">
          <a href="../themen/<?= e($art['slug']) ?>" target="_blank" class="icon-btn" title="Vorschau im Frontend">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
          </a>
          <a href="artikel-editor.php?id=<?= (int)$art['id'] ?>" class="icon-btn" title="Bearbeiten">
